...
func (s *Service) BootstrapStream(stream bpb.Bootstrap_BootstrapStreamServer) error {
	...
	for {
		...
		case *bpb.BootstrapStreamRequest_BootstrapRequest:
			...
			lu, err := buildEntityLookup(ctx, bootstrapReq, nil)
			if err != nil {
		...
		case *bpb.BootstrapStreamRequest_ReportStatusRequest:
			...
			if session.currentState == stateInitial {
				...
				lu, err := buildEntityLookup(ctx, nil, req.ReportStatusRequest)
				if err != nil {
					return err
				}
...
func (s *Service) BootstrapStreamV1(stream bpb.Bootstrap_BootstrapStreamV1Server) error {
	...
	for {
		...
		case *bpb.BootstrapStreamRequestV1_BootstrapRequest:
			...
			lu, err := buildEntityLookup(ctx, bootstrapRequest, nil)
			if err != nil {
				return status.Errorf(codes.InvalidArgument, "failed to build entity lookup from request: %v", err)
			}
...
// buildEntityLookup constructs an EntityLookup object from a GetBootstrapDataRequest message or a ReportStatusRequest message.
func buildEntityLookup(ctx context.Context, req *bpb.GetBootstrapDataRequest, sta *bpb.ReportStatusRequest) (*types.EntityLookup, error) {
	peerAddr, err := peerAddressFromContext(ctx)
	if err != nil {
		return nil, err
	}
	...
	switch {
	case req != nil:
		cc = req.GetControlCardState()
		id = req.GetIdentity()
		if cards := req.GetChassisDescriptor().GetControlCards(); len(cards) > 0 { // Modular chassis
			for _, v := range cards {
				serials = append(serials, v.GetSerialNumber())
			}
		} else { // Fixed form factor chassis
			serials = append(serials, req.GetChassisDescriptor().GetSerialNumber())
		}
	case sta != nil:
		if ccs := sta.GetStates(); len(ccs) > 0 {
